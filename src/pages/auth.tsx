...
import { generateSerialNumber, normalizeSerial } from "@/lib/serial-auth";
...
export default function AuthPage() {
  ...
  useEffect(() => {
    const onNoAccount = () => {
      toast({
        ...
        description: "هذا الإيميل غير مسجّل في المنصة. تواصل مع الإدارة للحصول على حساب.",
        variant: "destructive",
      });
    };
    window.addEventListener("cemgm:google-no-account", onNoAccount);
    return () => window.removeEventListener("cemgm:google-no-account", onNoAccount);
  }, [toast]);
  ...
  const handleOAuthOnboardingSubmit = async (e: React.FormEvent) => {
    ...
    // Serial login is healed on the next serial sign-in if needed.
          ...
          {mode === "login" && (
            <motion.form key="login" initial={{ x: -20, opacity: 0 }} animate={{ x: 0, opacity: 1 }} exit={{ x: 20, opacity: 0 }} onSubmit={handleLogin} className="space-y-6">
              ...
              <Button
                ...
                disabled={loading}
                className="w-full h-12 text-base font-medium bg-background hover:bg-secondary border-border flex items-center justify-center gap-3"
              >
              ...
              <p className="text-[11px] text-center text-muted-foreground -mt-3">
                الدخول عبر Google متاح فقط للحسابات المربوطة مسبقاً.
              </p>
